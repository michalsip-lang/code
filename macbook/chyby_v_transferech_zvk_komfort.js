(function() {
  ...
  function lookupSuggestionMatchesExpected(internalName, suggestionText, expectedText) {
    var expected = trimSafe(expectedText || "").toUpperCase();
    var suggestion = trimSafe(suggestionText || "");
    if (!expected || !suggestion) return false;

    // Pro DSU je vyžadována přesná shoda čísla, aby se nevybral "podobný" záznam.
    if (internalName === "cislo_dl") {
      var leadTokenMatch = suggestion.match(/^[^\s|,;:\-()\[\]]+/);
      var leadToken = trimSafe(leadTokenMatch ? leadTokenMatch[0] : suggestion).toUpperCase();
      return leadToken === expected;
    }

    return suggestion.toUpperCase().indexOf(expected) !== -1;
  }

  function selectFirstAutocompleteSuggestionViaJquery(el, expectedText, internalName) {
    if (!el || !window.jQuery) return false;
    ...
    try {
      ...
      var visibleItems = widget && widget.length ? widget.find("li:visible") : null;
      if (!visibleItems || !visibleItems.length) return false;

      var firstItem = null;
      var itemData = null;

      for (var vi = 0; vi < visibleItems.length; vi++) {
        var candidate = window.jQuery(visibleItems[vi]);
        var candidateData = candidate.data("ui-autocomplete-item") || candidate.data("item.autocomplete") || null;
        var candidateText = candidateData
          ? (candidateData.label || candidateData.value || "")
          : trimSafe(candidate.text() || "");

        if (lookupSuggestionMatchesExpected(internalName, candidateText, expectedText)) {
          firstItem = candidate;
  ...
  function trySelectFirstAutocompleteSuggestion(internalName, expectedText, attempt) {
    ...
    if (fieldCtrl && selectFirstAutocompleteSuggestionViaJquery(fieldCtrl, expectedText, internalName)) {
      setTimeout(function() {
    ...
    for (var ni = 0; ni < nodes.length; ni++) {
      var nodeText = trimSafe(nodes[ni].textContent || nodes[ni].innerText || "");
      if (lookupSuggestionMatchesExpected(internalName, nodeText, expectedText)) {
        firstNode = nodes[ni];
        break;
      }
    }
  ...
  function syncTocHtmlFromAdvancedTable(panel) {
    ...
    // Rozdělíme řádky na BRAMB, CENTR a ostatní větve.
    var rowsBramb = [];
    var rowsCentr = [];
    var rowsOther = [];

    for (var ri = 0; ri < rows.length; ri++) {
      var locationCell = rows[ri] && Array.isArray(rows[ri].Cells) ? trimSafe(rows[ri].Cells[COL_LOCATION] || "") : "";
      var locationKey = getBaseLocationKey(locationCell);
      if (locationKey === "BRAMB") {
        rowsBramb.push(rows[ri]);
      } else if (locationKey === "CENTR") {
        rowsCentr.push(rows[ri]);
      } else {
        rowsOther.push(rows[ri]);
      }
    }

    var hasBramb = rowsBramb.length > 0;
    var hasCentr = rowsCentr.length > 0;
    var hasOther = rowsOther.length > 0;
    var splitView = hasBramb && hasCentr;

    var htmlTable = "";

    function sectionHeading(label) {
      return "<div style='font-family:Segoe UI,Arial,sans-serif;font-size:14px;font-weight:600;margin-top:16px;margin-bottom:6px;'>" +
        label + "</div>";
    }

    if (splitView) {
      // Dvě tabulky – BRAMB a CENTR, OSTATNÍ vždy jako samostatná třetí tabulka.
      htmlTable +=
        sectionHeading("Bramborárna (BRAMB)") +
        advancedTableRowsToEmailHtml(rowsBramb) +
        sectionHeading("Centrála (CENTR)") +
        advancedTableRowsToEmailHtml(rowsCentr);
      if (hasOther) {
        htmlTable += sectionHeading("Ostatní") + advancedTableRowsToEmailHtml(rowsOther);
      }
    } else if (hasBramb && hasOther) {
      htmlTable +=
        sectionHeading("Bramborárna (BRAMB)") +
        advancedTableRowsToEmailHtml(rowsBramb) +
        sectionHeading("Ostatní") +
        advancedTableRowsToEmailHtml(rowsOther);
    } else if (hasCentr && hasOther) {
      htmlTable +=
        sectionHeading("Centrála (CENTR)") +
        advancedTableRowsToEmailHtml(rowsCentr) +
        sectionHeading("Ostatní") +
      ...
      htmlTable = advancedTableRowsToEmailHtml(rows);
    }

    // Sbíráme čísla zboží z každé lokace pro Navision.
    function extractGoodsNumbers(rowArr) {
      var goods = [];
      for (var gri = 0; gri < rowArr.length; gri++) {
        var goodsCell = rowArr[gri] && Array.isArray(rowArr[gri].Cells) ? trimSafe(rowArr[gri].Cells[COL_GOODS] || "") : "";
        if (goodsCell) {
          // Odstraníme lookup info pokud je přítomen.
          var baseGoods = stripLookupInfo(goodsCell);
          if (baseGoods) goods.push(baseGoods);
        }
      }
      return goods;
    }
    ...
    var goodsListHtml = "";
    var bramb = extractGoodsNumbers(rowsBramb);
    var centr = extractGoodsNumbers(rowsCentr);
    var other = extractGoodsNumbers(rowsOther);

    if (bramb.length > 0 || centr.length > 0 || other.length > 0) {
      goodsListHtml += "<div style='margin-top:20px;font-family:Segoe UI,Arial,sans-serif;font-size:13px;'>";
      goodsListHtml += "<div style='font-weight:600;margin-bottom:8px;'>Čísla zboží pro inventuru (Navision):</div>";

      if (bramb.length > 0) {
        goodsListHtml += "<div style='margin-bottom:6px;'><strong>BRAMB:</strong> " +
          bramb.join("|") + "</div>";
      }
      if (centr.length > 0) {
        goodsListHtml += "<div style='margin-bottom:6px;'><strong>CENTR:</strong> " +
          centr.join("|") + "</div>";
      }
      if (other.length > 0) {
        goodsListHtml += "<div style='margin-bottom:6px;'><strong>OSTATNÍ:</strong> " +
          other.join("|") + "</div>";
      }
    ...
    return setFieldHtml(TOC_HTML_FIELD, htmlTable + goodsListHtml + komentarHtml);
  }
  ...
  function openTaskFormNoteModal(options) {
    ...
    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var saveButton = dialog.querySelector("#taskFormNoteSave");
        if (saveButton) saveButton.click();
      },
      onEscape: function() {
        var cancelButton = dialog.querySelector("#taskFormNoteCancel");
  ...
  function openDecisionChangeConfirmModal(options) {
    ...
    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var confirmBtn = dialog.querySelector("#taskFormDecisionChangeConfirm");
        if (confirmBtn) confirmBtn.click();
      },
      onEscape: function() {
        var cancelBtn = dialog.querySelector("#taskFormDecisionChangeCancel");
  ...
  var COMFORT_DRAFT_STORAGE_KEY = "chyby_transfer_comfort_draft_v1";
  ...
  function readComfortDraft() {
    var storage = getSessionStorageSafe();
    if (!storage) return {};

    try {
      var raw = storage.getItem(COMFORT_DRAFT_STORAGE_KEY);
      if (!raw) return {};
  ...
  function writeComfortDraft(partial) {
    var storage = getSessionStorageSafe();
    if (!storage || !partial || typeof partial !== "object") return;

    var merged = readComfortDraft();
    var keys = Object.keys(partial);

    for (var i = 0; i < keys.length; i++) {
      merged[keys[i]] = partial[keys[i]];
    }

    try {
      storage.setItem(COMFORT_DRAFT_STORAGE_KEY, JSON.stringify(merged));
    } catch (e) {}
  }

  function removeComfortDraftKeys(keysToRemove) {
    var storage = getSessionStorageSafe();
    if (!storage) return;

    var draft = readComfortDraft();
    var hasAny = false;

    for (var i = 0; i < keysToRemove.length; i++) {
      if (Object.prototype.hasOwnProperty.call(draft, keysToRemove[i])) {
        delete draft[keysToRemove[i]];
        hasAny = true;
      }
    }
    ...
    try {
      if (Object.keys(draft).length === 0) {
        storage.removeItem(COMFORT_DRAFT_STORAGE_KEY);
      } else {
        storage.setItem(COMFORT_DRAFT_STORAGE_KEY, JSON.stringify(draft));
      }
    } catch (e) {}
  }

  function bindModalKeyboardShortcuts(dialog, options) {
    if (!dialog || dialog.__keyboardShortcutsBound) return;
    dialog.__keyboardShortcutsBound = true;
    options = options || {};
  ...
  function updateNewFormDeliveryLoadingModalState(state) {
    state = state || {};

    var stepNode = document.getElementById("newFormDeliveryLoadingStep");
    var detailNode = document.getElementById("newFormDeliveryLoadingDetail");
    var progressNode = document.getElementById("newFormDeliveryLoadingProgress");
    var barNode = document.getElementById("newFormDeliveryLoadingBar");
    var retryBtn = document.getElementById("newFormDeliveryRetry");
    var editBtn = document.getElementById("newFormDeliveryEdit");

    if (stepNode && typeof state.stepText === "string") stepNode.textContent = state.stepText;
    if (detailNode && typeof state.detailText === "string") detailNode.textContent = state.detailText;

    if (progressNode && typeof state.progressText === "string") {
      progressNode.textContent = state.progressText;
    }

    if (barNode && typeof state.progressPercent === "number") {
      var clamped = Math.max(5, Math.min(100, state.progressPercent));
      barNode.style.width = clamped + "%";
    }

    if (retryBtn) {
      retryBtn.style.display = state.showRetry ? "inline-block" : "none";
      retryBtn.disabled = !!state.retryDisabled;
      if (state.retryLabel) retryBtn.textContent = state.retryLabel;
    }

    if (editBtn) {
      editBtn.style.display = state.showEdit ? "inline-block" : "none";
      editBtn.disabled = !!state.editDisabled;
      if (state.editLabel) editBtn.textContent = state.editLabel;
    }
  }

  function reopenNewFormDeliveryNoteAfterTimeout(message) {
    clearNewFormDeliveryLookupWaiter();
    hideNewFormDeliveryNoteLoadingModal();

    setFieldValueSmart("cislo_dl", "");

    window.__newFormDeliveryNoteConfirmed = false;
    window.__pendingContinueAfterDeliveryNote = false;
    window.__newFormDeliveryLookupFailed = true;
    window.__typeModalShownAfterDsu = false;
    window.__newFormDeliveryPromptError = trimSafe(message || "");
  ...
  function collectProdejnaRowsFromPanel(panel) {
    if (!panel) return [];
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var skladTarget = getDecisionTargetInRow(rows[i], TASKFORM_COL_STAV_SKLAD);
      var prodejnaTarget = getDecisionTargetInRow(rows[i], TASKFORM_COL_STAV_PRODEJNA);

      if (!goodsNumber) continue;

      // Zajímají nás jen řádky kde sklad = nepotvrzen a prodejna ještě nevyplněna.
      var skladCanonical = toCanonicalDecisionValue(getDecisionControlRawValue(skladTarget));
      if (skladCanonical !== "rozdil_nepotvrzen") continue;
      if (isDecisionControlFilled(prodejnaTarget)) continue;
  ...
  function applyProdejnaDecisionsToRows(panel, decisions) {
    if (!panel || !Array.isArray(decisions) || decisions.length === 0) return false;
    ...
    for (var i = 0; i < decisions.length; i++) {
      var decision = decisions[i] || {};
      var tr = rows[decision.rowIndex];
      var target = getDecisionTargetInRow(tr, TASKFORM_COL_STAV_PRODEJNA);

      if (!target || !decision.label) {
        hadFailure = true;
        continue;
      }
  ...
  function showProdejnaDecisionModal(options) {
    options = options || {};

    var existing = document.getElementById("prodejnaDecisionBackdrop");
    if (existing && existing.parentNode) existing.parentNode.removeChild(existing);

    var items = Array.isArray(options.items) ? options.items : [];
    var backdrop = document.createElement("div");
    backdrop.id = "prodejnaDecisionBackdrop";
    backdrop.style.cssText =
      "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.35);" +
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i] || {};
      var rowName = "prodejnaDecision_" + item.rowIndex;
      rowsHtml.push(
        "<tr>" +
        ...
        "<label style='display:inline-flex;align-items:center;gap:6px;margin-right:14px;cursor:pointer;'><input type='radio' name='" + rowName + "' value='trvame'>Prodejna trvá na rozdílu</label>" +
        "<label style='display:inline-flex;align-items:center;gap:6px;cursor:pointer;'><input type='radio' name='" + rowName + "' value='nepotvrdi'>Prodejna rozdíl nepotvrdila</label>" +
        "</td>" +
        "</tr>"
      );
    }
    ...
    dialog.innerHTML =
      ...
      "  <h2 style='margin:0 0 6px 0;font-size:18px;color:#004578;'>Vyjádření prodejny</h2>" +
      "</div>" +
      "<div style='font-size:12px;color:#605e5c;margin-bottom:10px;'>Níže jsou položky, u kterých sklad rozdíl nepotvrzen. Vyjádři se za prodejnu ke každé položce.</div>" +
      "<div style='border:1px solid #d0d0d0;border-radius:6px;overflow:hidden;'>" +
      "  <table style='width:100%;border-collapse:collapse;font-size:13px;'>" +
      "    <thead><tr style='background:#f3f2f1;'><th style='border:1px solid #d0d0d0;padding:8px;text-align:left;'>Číslo zboží</th><th style='border:1px solid #d0d0d0;padding:8px;text-align:left;'>Lokace</th><th style='border:1px solid #d0d0d0;padding:8px;text-align:left;'>Vyjádření prodejny</th></tr></thead>" +
      "    <tbody>" + rowsHtml.join("") + "</tbody>" +
      "  </table>" +
      "</div>" +
      "<div id='prodejnaDecisionErr' style='display:none;margin-top:10px;color:#a80000;font-size:12px;'></div>" +
      "<div style='display:flex;justify-content:flex-end;gap:8px;margin-top:14px;'>" +
      "  <button type='button' id='prodejnaDecisionCancel' style='padding:8px 12px;border-radius:4px;border:1px solid #8a8886;background:#fff;cursor:pointer;'>Zrušit</button>" +
      "  <button type='button' id='prodejnaDecisionSave' style='padding:8px 12px;border-radius:4px;border:1px solid #0078d4;background:#0078d4;color:#fff;cursor:pointer;font-weight:600;'>Zapsat vyjádření prodejny</button>" +
      "</div>";
    ...
    dialog.querySelector("#prodejnaDecisionCancel").onclick = close;

    dialog.querySelector("#prodejnaDecisionSave").onclick = function() {
      var err = dialog.querySelector("#prodejnaDecisionErr");
      var decisions = [];

      for (var i = 0; i < items.length; i++) {
        var item = items[i] || {};
        var selected = dialog.querySelector("input[name='prodejnaDecision_" + item.rowIndex + "']:checked");
        ...
        decisions.push({
          ...
          label: selected.value === "trvame" ? "Prodejna trvá na rozdílu" : "Prodejna rozdíl nepotvrdila"
        });
      }
      ...
      if (ok === false) {
        err.textContent = "Nepodařilo se zapsat vyjádření do tabulky.";
        err.style.display = "block";
        return;
      }
    ...
    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var saveBtn = dialog.querySelector("#prodejnaDecisionSave");
        if (saveBtn) saveBtn.click();
      },
      onEscape: function() {
        var cancelBtn = dialog.querySelector("#prodejnaDecisionCancel");
        if (cancelBtn) cancelBtn.click();
      }
    });
  ...
  function getDatabaseDeliveryNumbers(csvText) {
    ...
    for (var i = 0; i < rows.length; i++) {
      var row = rows[i] || [];
      var dsuNumber = trimSafe(row[0] || "").toUpperCase();
      if (!dsuNumber || seen[dsuNumber]) continue;
      seen[dsuNumber] = true;
      numbers.push(dsuNumber);
    }
  ...
  function getTransferDataFromDatabase(csvText) {
    var rows = parseDatabaseCsvRows(csvText);
    if (!rows.length) {
      return { ok: false, message: "DSU neobsahuje žádná data.", transferNumbers: [], byGoods: {} };
    }

    var byGoods = {};
    var transferNumbersMap = {};
    var transferNumbers = [];

    for (var i = 0; i < rows.length; i++) {
      var row = rows[i] || [];
      if (row.length < 5) {
        return { ok: false, message: "Řádek " + (i + 1) + " v DSU není kompletní (očekáváno 5 sloupců).", transferNumbers: [], byGoods: {} };
      }

      for (var c = 0; c < 5; c++) {
        if (!trimSafe(row[c] || "")) {
          return { ok: false, message: "Řádek " + (i + 1) + " v DSU není kompletní (sloupec " + (c + 1) + " je prázdný).", transferNumbers: [], byGoods: {} };
        }
      }

      var transferNumber = trimSafe(row[2] || "");
      var goodsNumber = normalizeDsuGoodsNumber(row[3] || "");
      var quantityDl = trimSafe(row[4] || "");

      if (transferNumber && !transferNumbersMap[transferNumber]) {
        transferNumbersMap[transferNumber] = true;
        transferNumbers.push(transferNumber);
      }

      if (goodsNumber && !byGoods[goodsNumber]) {
        byGoods[goodsNumber] = { quantityDl: quantityDl, transferNumber: transferNumber };
      }
    }

    return { ok: true, transferNumbers: transferNumbers, byGoods: byGoods };
  }
  ...
  function showNewFormDeliveryNoteModal(options) {
    ...
    dialog.innerHTML =
      ...
      "<div style='margin-top:10px;padding:8px 10px;border:1px solid #edebe9;border-radius:6px;background:#faf9f8;'>" +
      "  <div style='font-size:12px;font-weight:600;margin-bottom:6px;'>Co otevřít po načtení DSU</div>" +
      "  <label style='display:block;font-size:12px;cursor:pointer;margin-bottom:4px;'><input type='radio' name='newFormPreferredType' value='rozdil' style='margin-right:6px;'>Rovnou Rozdíl v transferu</label>" +
      "  <label style='display:block;font-size:12px;cursor:pointer;'><input type='radio' name='newFormPreferredType' value='problem' style='margin-right:6px;'>Rovnou Problém s transferem</label>" +
      "</div>" +
      "<div id='newFormDeliveryNoteErr' style='display:none;margin-top:8px;color:#a80000;font-size:12px;'></div>" +
      "<div style='display:flex;justify-content:flex-end;gap:8px;margin-top:14px;'>" +
      "  <button type='button' id='newFormDeliveryNoteCancel' style='padding:8px 12px;border-radius:4px;border:1px solid #8a8886;background:#fff;color:#323130;cursor:pointer;'>Zrušit</button>" +
    ...
    var cancelBtn = dialog.querySelector("#newFormDeliveryNoteCancel");
    var preferredType = trimSafe(draft.preferredType || "rozdil");
    var initialError = trimSafe(options.initialError || window.__newFormDeliveryPromptError || "");
    var preferredTypeSelector = "input[name='newFormPreferredType'][value='" + (preferredType === "problem" ? "problem" : "rozdil") + "']";
    var preferredRadio = dialog.querySelector(preferredTypeSelector);
    if (preferredRadio) preferredRadio.checked = true;

    if (trimSafe(draft.deliveryNumber || "")) {
      valueInput.value = trimSafe(draft.deliveryNumber || "");
    }

    if (initialError) {
      err.textContent = initialError;
      err.style.display = "block";
      window.__newFormDeliveryPromptError = "";
    }
    ...
    saveBtn.onclick = function() {
      ...
      var normalizedDeliveryNumber = deliveryNumber.toUpperCase();
      var selectedTypeNode = dialog.querySelector("input[name='newFormPreferredType']:checked");
      var selectedType = selectedTypeNode ? trimSafe(selectedTypeNode.value || "") : "rozdil";
      if (!deliveryNumber) {
      ...
      // Tvrdá validace proti falešným lookup shodám (např. jen "5").
      if (normalizedDeliveryNumber.indexOf("DSU") !== 0 || normalizedDeliveryNumber.length < 6) {
        err.textContent = "Číslo DSU musí začínat textem DSU a obsahovat minimálně 3 další znaky.";
        err.style.display = "block";
        valueInput.focus();
        return;
      }
      ...
      writeComfortDraft({
        ...
        preferredType: selectedType === "problem" ? "problem" : "rozdil"
      });
      window.__newFormPreferredType = selectedType === "problem" ? "problem" : "rozdil";
      ...
      ensureDeliveryLookupAndWaitDatabase(deliveryNumber, {
        ...
        onMismatch: function(message) {
          updateNewFormDeliveryLoadingModalState({
            stepText: "Krok 2 z 3: Ověření čísla DSU",
            detailText: trimSafe(message || "Číslo DSU se neshoduje s načtenými daty. Zkontrolujte zadanou hodnotu."),
            progressText: "Neplatné DSU",
            progressPercent: 100,
            showRetry: true,
            retryDisabled: false,
            ...
            showEdit: true,
            editDisabled: false,
            editLabel: "Upravit DSU"
          });
        },
        onTimeout: function() {
          updateNewFormDeliveryLoadingModalState({
            stepText: "Krok 2 z 3: Čekání na doplnění database",
            detailText: "DSU nebylo nalezeno nebo se nepodařilo načíst data. Můžete lookup zopakovat nebo upravit DSU.",
            progressText: "Vypršel časový limit",
            progressPercent: 100,
            showRetry: true,
            retryDisabled: false,
            retryLabel: "Zkusit znovu",
  ...
  function showNewFormDeliveryNoteLoadingModal() {
    ...
    dialog.innerHTML =
      ...
      "</div>" +
      "<div id='newFormDeliveryLoadingStep' style='font-size:13px;color:#323130;line-height:1.45;font-weight:600;'>Krok 1 z 3: Ověření DSU</div>" +
      "<div id='newFormDeliveryLoadingDetail' style='font-size:12px;color:#323130;line-height:1.45;margin-top:4px;'>Spouštím lookup pro cislo_dl.</div>" +
      "<div style='margin-top:10px;height:8px;border-radius:999px;background:#edebe9;overflow:hidden;'><span id='newFormDeliveryLoadingBar' style='display:block;height:100%;width:8%;background:#0078d4;'></span></div>" +
      "<div id='newFormDeliveryLoadingProgress' style='font-size:11px;color:#605e5c;margin-top:6px;'>Probíhá načítání...</div>" +
      "<div style='display:flex;justify-content:center;gap:8px;margin-top:12px;'>" +
      "  <button type='button' id='newFormDeliveryEdit' style='display:none;padding:6px 10px;border-radius:4px;border:1px solid #8a8886;background:#fff;color:#323130;cursor:pointer;'>Upravit DSU</button>" +
      "  <button type='button' id='newFormDeliveryRetry' style='display:none;padding:6px 10px;border-radius:4px;border:1px solid #0078d4;background:#0078d4;color:#fff;cursor:pointer;'>Zkusit znovu</button>" +
      "</div>";
    ...
    var retryBtn = dialog.querySelector("#newFormDeliveryRetry");
    var editBtn = dialog.querySelector("#newFormDeliveryEdit");
    if (retryBtn) {
      retryBtn.onclick = function() {
        if (typeof window.__retryDeliveryLookup === "function") {
          retryBtn.disabled = true;
          window.__retryDeliveryLookup();
        }
      };
    }
    ...
    if (editBtn) {
      editBtn.onclick = function() {
        reopenNewFormDeliveryNoteAfterTimeout("Zadané DSU se nepodařilo načíst. Zkontrolujte číslo dodacího listu a zkuste to znovu.");
      };
    }

    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var btn = dialog.querySelector("#newFormDeliveryRetry");
        if (btn && btn.style.display !== "none" && !btn.disabled) btn.click();
      }
    });

    updateNewFormDeliveryLoadingModalState({
      stepText: "Krok 1 z 3: Ověření DSU",
      detailText: "Spouštím lookup pro cislo_dl.",
      progressText: "Probíhá načítání...",
      progressPercent: 8,
      showRetry: false,
      showEdit: false
    });
  ...
  function ensureDeliveryLookupAndWaitDatabase(deliveryNumber, options) {
    ...
    window.__newFormDeliveryLookupRunning = true;
    window.__lastDeliveryLookupValue = deliveryNumber;
    window.__retryDeliveryLookup = function() {
      if (window.__newFormDeliveryLookupRunning) return;
      ensureDeliveryLookupAndWaitDatabase(window.__lastDeliveryLookupValue || deliveryNumber, options);
    };

    if (!ctrl) {
      hideNewFormDeliveryNoteLoadingModal();
      window.__newFormDeliveryLookupRunning = false;
    ...
    function succeed(databaseValue) {
      ...
      window.__newFormDeliveryLookupFailed = false;
      window.__newFormDeliveryLookupRunning = false;
      removeComfortDraftKeys(["deliveryNumber"]);
    ...
    function fail(reason) {
      ...
      window.__pendingContinueAfterDeliveryNote = false;
      window.__newFormDeliveryLookupFailed = true;
      if (reason === "mismatch" && typeof options.onMismatch === "function") {
        options.onMismatch(window.__newFormDeliveryMismatchMessage || "");
        return;
      }
      if (typeof options.onTimeout === "function") options.onTimeout();
    }
    ...
    function tick() {
      ...
      var progressPercent = Math.min(96, Math.round((elapsedMs / maxWaitMs) * 100));
      var progressSec = Math.max(0, Math.floor((maxWaitMs - elapsedMs) / 1000));
      ...
      if (currentDatabaseValue && (currentDatabaseValue !== initialDatabaseValue || !initialDatabaseValue)) {
        var expectedDsu = trimSafe(deliveryNumber || "").toUpperCase();
        var dsuNumbers = getDatabaseDeliveryNumbers(currentDatabaseValue);
        var hasExactDsu = false;
        for (var dn = 0; dn < dsuNumbers.length; dn++) {
          if (dsuNumbers[dn] === expectedDsu) {
            hasExactDsu = true;
            break;
          }
        }
        ...
        if (!hasExactDsu) {
          window.__newFormDeliveryMismatchMessage =
            "Zadané DSU " + trimSafe(deliveryNumber || "") + " se neshoduje s načtenými daty (nalezeno: " +
            (dsuNumbers.length ? dsuNumbers.join(", ") : "žádné číslo") + ").";
          fail("mismatch");
          return;
        }

        updateNewFormDeliveryLoadingModalState({
          stepText: "Krok 3 z 3: Připravuji formulář",
          detailText: "Pole database je vyplněno. Dokončuji přechod.",
          progressText: "Dokončeno",
          progressPercent: 100,
          showRetry: false
        });
      ...
      updateNewFormDeliveryLoadingModalState({
        stepText: attempt < 3 ? "Krok 1 z 3: Ověření DSU" : "Krok 2 z 3: Čekání na doplnění database",
        detailText: attempt < 3
          ? "Spouštím lookup pro cislo_dl."
          : "Čekám na vyplnění pole database z lookupu.",
        progressText: "Zbývá přibližně " + progressSec + " s",
  ...
  function initDatabaseField(attempt) {
    ...
    function tryContinueAfterDsu(showWarnings) {
      ...
      var transferData = getTransferDataFromDatabase(databaseValue);
      if (!transferData.ok) {
        if (showWarnings) {
          showCenteredWarningModal(
            "Neplatné DSU",
            transferData.message || "Nepodařilo se načíst data z DSU."
          );
        }
        return false;
      }

      var allTransferNumbers = transferData.transferNumbers.join(", ");
      var transferSetOk = setFieldValueSmart("cislo_transferu", allTransferNumbers);
      if (!transferSetOk) {
        if (showWarnings) {
          showCenteredWarningModal(
            "Nelze doplnit číslo transferu",
            "Nepodařilo se vyplnit pole cislo_transferu hodnotou " + allTransferNumbers + "."
          );
        }
        return false;
      }
      ...
      window.__pendingContinueAfterDeliveryNote = false;
      hideNewFormDeliveryNoteLoadingModal();
      var preferredType = trimSafe(window.__newFormPreferredType || "") || trimSafe(readComfortDraft().preferredType || "");
      if (!applyTypeSelectionAndContinue(preferredType)) {
        createTypeModal();
      }
      return true;
    }
  ...
  function showTaskInventoryAssignmentModal(options) {
    ...
    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var saveBtn = dialog.querySelector("#taskInventoryAssignmentSave");
        if (saveBtn) saveBtn.click();
      },
      onEscape: function() {
        var cancelBtn = dialog.querySelector("#taskInventoryAssignmentCancel");
        if (cancelBtn) cancelBtn.click();
      }
    });
  ...
  function showTaskRoleModalOnTaskForm(attempt) {
    ...
    resolveUserLocationBranch(function(resolveErr, userInfo) {
      ...
      var userCenter = trimSafe(userInfo && userInfo.center ? userInfo.center : "");
      var hasKnownUserCenter = !!userCenter;
      var hasSupportedUserBranch = !!userBranch;
      var detectedBranch = detectTaskLocationBranchFromPanel(panel);

      // Pro inventuru skladu se branch řídí střediskem uživatele (bez fallbacku na úkol).
      // Pro vyjádření prodejny se vždy zobrazí veškeré zboží z úkolu (detectedBranch).
      var effectiveBranch = role === "inventura_skladu"
        ? (hasSupportedUserBranch ? userBranch : (hasKnownUserCenter ? "" : detectedBranch))
        : detectedBranch;
      var goodsForUser = collectGoodsForLocationBranch(panel, effectiveBranch);

      if (role === "inventura_skladu") {
        if (hasKnownUserCenter && !hasSupportedUserBranch) {
          showCenteredWarningModal(
            "Pro vaše středisko není dostupné zboží k inventuře",
            "Pro vaše středisko " + userCenter + " není dostupné žádné zboží k inventuře."
          );
          return;
        }
      ...
      if (role === "vyjadreni_prodejny") {
        var prodejnaItems = collectProdejnaRowsFromPanel(panel);
        if (prodejnaItems.length === 0) {
          showCenteredWarningModal(
            "Všechno zboží je již zpracováno",
            "Pro tento úkol nejsou žádné položky k vyjádření prodejny (všechny jsou již vyplněny nebo sklad potvrdil rozdíl)."
          );
          return;
        }

        showProdejnaDecisionModal({
          items: prodejnaItems,
          onSave: function(decisions) {
            return applyProdejnaDecisionsToRows(panel, decisions);
          }
        });
        return;
      }
  ...
  function createTocEditorModal() {
    ...
    var draft = readComfortDraft();
    var draftRows = Array.isArray(draft.tocDraftRows) ? draft.tocDraftRows : [];
    var sourceRows = existing.length > 0 ? existing : draftRows;
    var databaseCtrl = getWritableFieldControl("database");
    ...
    for (var d = 0; d < databaseRows.length; d++) {
      ...
      dsuByGoods[dsuGoods] = {
        ...
        transferNumber: trimSafe(dsuCells[2] || "")
      };
    }
    ...
    dialog.innerHTML =
      ...
      "      <th style='text-align:left;border-bottom:1px solid #edebe9;padding:8px;'>Číslo transferu</th>" +
    ...
    function resetRowValidation(tr) {
      if (!tr) return;

      tr.style.backgroundColor = "";
      var invalidNodes = tr.querySelectorAll("[data-toc-invalid='1']");
      for (var i = 0; i < invalidNodes.length; i++) {
        invalidNodes[i].style.borderColor = "";
        invalidNodes[i].style.backgroundColor = "";
        invalidNodes[i].removeAttribute("title");
        invalidNodes[i].removeAttribute("data-toc-invalid");
      }
    }

    function markRowValidation(tr, inputs, message) {
      if (!tr) return;
      tr.style.backgroundColor = "#fff4f4";

      for (var i = 0; i < inputs.length; i++) {
        if (!inputs[i]) continue;
        inputs[i].style.borderColor = "#a4262c";
        inputs[i].style.backgroundColor = "#fff4f4";
        inputs[i].setAttribute("title", message || "Neplatná hodnota.");
        inputs[i].setAttribute("data-toc-invalid", "1");
      }
    }

    function collectRowsDraft() {
      var rows = [];
      var trs = dialog.querySelectorAll("#tocBody tr");

      for (var i = 0; i < trs.length; i++) {
        var tr = trs[i];
        rows.push({
          Cells: normalizeCells([
            trimSafe(tr.querySelector(".toc-cislo").value || ""),
            trimSafe(tr.querySelector(".toc-lokace").value || ""),
            trimSafe(tr.querySelector(".toc-dl").value || ""),
            trimSafe(tr.querySelector(".toc-fiz").value || ""),
            trimSafe(tr.querySelector(".toc-rozdil").value || "")
          ])
        });
      }
    ...
    function renderRowFromCells(cells) {
      ...
      tr.innerHTML =
        ...
        "<td style='padding:8px;'><input type='text' class='toc-transfer' style='width:100%;box-sizing:border-box;padding:6px;background:#f3f2f1;min-width:110px;' value='' readonly disabled></td>" +
      ...
      function syncGoodsFromDsu() {
        ...
        var transferInput = tr.querySelector(".toc-transfer");
        if (transferInput) transferInput.value = dsuMatch.transferNumber || "";
      }
      ...
      tr.querySelector(".toc-dl").addEventListener("input", function() {
        resetRowValidation(tr);
        recalc();
        scheduleDraftSave();
      });
      tr.querySelector(".toc-fiz").addEventListener("input", function() {
        resetRowValidation(tr);
        recalc();
        scheduleDraftSave();
      });
    ...
    if (sourceRows.length > 0) {
      for (var i = 0; i < sourceRows.length; i++) {
        document.getElementById("tocBody").appendChild(renderRowFromCells(sourceRows[i].Cells));
      }
    } else {
      addEmptyRow();
    }
    ...
    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var saveButton = document.getElementById("tocSave");
        if (saveButton && !saveButton.disabled) saveButton.click();
      },
      onEscape: function() {
        var cancelButton = document.getElementById("tocCancel");
        if (cancelButton) cancelButton.click();
      }
    });
    ...
    document.getElementById("tocSave").addEventListener("click", function() {
      ...
      for (var i = 0; i < trs.length; i++) {
        ...
        if (!cislo) {
          ...
          markRowValidation(tr, [tr.querySelector(".toc-cislo")], "Řádek musí obsahovat číslo zboží.");
          showError("Řádek " + (i + 1) + ": chybí Číslo zboží.");
          tr.querySelector(".toc-cislo").focus();
          return;
        }
        ...
        if (!dl || !fiz) {
          ...
          markRowValidation(
            tr,
            [!dl ? tr.querySelector(".toc-dl") : null, !fiz ? tr.querySelector(".toc-fiz") : null],
            "Doplňte povinné hodnoty na řádku."
          );
        ...
        var transferInput = tr.querySelector(".toc-transfer");
        var transferNum = trimSafe(transferInput ? (transferInput.value || "") : "");
      ...
      // Sbíráme unikátní čísla transferů ze skutečně vybraných řádků.
      var usedTransferMap = {};
      var usedTransferList = [];
      for (var ti = 0; ti < rows.length; ti++) {
        var tNum = rows[ti].transferNumber || "";
        if (tNum && !usedTransferMap[tNum]) {
          usedTransferMap[tNum] = true;
          usedTransferList.push(tNum);
        }
      }
      ...
      enrichRowsFromLookup(rows, function(enrichedRows, missingGoods) {
        if (missingGoods && missingGoods.length > 0) {
          ...
          var missingMap = {};
          for (var m = 0; m < missingGoods.length; m++) {
            missingMap[normalizeDsuGoodsNumber(missingGoods[m])] = true;
          }

          for (var t = 0; t < trs.length; t++) {
            var goodsInput = trs[t].querySelector(".toc-cislo");
            var key = normalizeDsuGoodsNumber(goodsInput && goodsInput.value ? goodsInput.value : "");
            if (!missingMap[key]) continue;
            markRowValidation(trs[t], [goodsInput], "Číslo zboží nebylo nalezeno v detail_o_zbozi.");
          }
        ...
        // Zapsat čísla transferů skutečně vybraného zboží.
        if (usedTransferList.length > 0) {
          setFieldValueSmart("cislo_transferu", usedTransferList.join(", "));
        }
        ...
        removeComfortDraftKeys(["tocDraftRows"]);
        closeEditor();
      });
    });
  ...
  function applyTypeSelectionAndContinue(typeKey) {
    var normalizedType = trimSafe(typeKey || "").toLowerCase();
    if (normalizedType !== "rozdil" && normalizedType !== "problem") return false;

    writeComfortDraft({
      preferredType: normalizedType
    });
    window.__newFormPreferredType = normalizedType;

    if (normalizedType === "rozdil") {
  ...
  function createTypeModal() {
    ...
    document.getElementById("btRozdil").onclick = function() {
      applyTypeSelectionAndContinue("rozdil");
      backdrop.remove();
    };

    // Problém s transferem -> zamknout sadu pro rozdíl
    document.getElementById("btProblem").onclick = function() {
      applyTypeSelectionAndContinue("problem");
      backdrop.remove();
    };

    bindModalKeyboardShortcuts(dialog, {
      onEnter: function() {
        var btn = document.getElementById("btRozdil");
        if (btn) btn.click();
      },
      onEscape: function() {
        if (backdrop.parentNode) backdrop.parentNode.removeChild(backdrop);
